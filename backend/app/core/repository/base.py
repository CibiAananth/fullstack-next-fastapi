...
class BaseRepository(Generic[ModelType]):
    ...
    async def _sort_by(
        ...
        if not hasattr(model, sort_by):
            raise AttributeError(f"{model} has no attribute {sort_by}")
    ...
    async def _add_join_to_query(self, query: Select, join_: str) -> Select:
        """
    ...
    async def _maybe_join(self, query: Select, join_: set[str] | None = None) -> Select:
        ...
        for join_operation in join_:
            query = await self._add_join_to_query(query, join_operation)
