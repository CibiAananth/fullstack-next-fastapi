...
class BaseController(Generic[ModelType]):
    ...
    async def get_by_id(self, id_: int, join_: set[str] | None = None) -> ModelType:
        ...
        if isinstance(db_obj, list):  # Handle the case where `a` is a list of strings
            return db_obj[0]  # Return the first element of the list
        return db_obj
    ...
    async def extract_attributes_from_schema(
        schema: BaseModel, excludes: Set[str] | None = None
    ) -> dict[str, Any]:
        """
