...
async def get_users(
    ...
    assert_access: Callable = Depends(Permissions(UserPermission.READ.value)),
) -> list[UserResponse]:
    users = await user_controller.get_all()

    assert_access(resource=users)
    print(users, [UserResponse.model_construct(**user.__dict__) for user in users])
    return [UserResponse.model_construct(**user.__dict__) for user in users]
